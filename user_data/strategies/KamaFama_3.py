...
class KamaFama_3(IStrategy):
    ...
    buy_mama_diff = DecimalParameter(
        0.01, 0.1, default=buy_params["buy_mama_diff"], decimals=2, space="buy", optimize=True
    )
    buy_cti = DecimalParameter(
        -1, 1, default=buy_params["buy_cti"], decimals=2, space="buy", optimize=True
    )
    buy_rsi_84 = IntParameter(0, 100, default=buy_params["buy_rsi_84"], space="buy", optimize=True)
    buy_rsi_112 = IntParameter(
        0, 100, default=buy_params["buy_rsi_112"], space="buy", optimize=True
    )

    # Sell Params
    sell_fastx = IntParameter(
        50, 100, default=sell_params["sell_fastx"], space="sell", optimize=True
    )
    ...
    plot_config = {
        ...
        "subplots": {"cond": {"change": {"color": "#da3e3e"}}},
    }
